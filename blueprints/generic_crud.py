...
def _load_menu_objects_for_menu(menustamp: str) -> list[dict]:
    ...
    for row in rows:
        ...
        object_type = str(row.get('TIPO') or 'TEXT').strip().upper()
        objects.append({
            ...
            'admin': False,
            'primary_key': False,
            'readonly': True if object_type == 'SEPARATOR' else bool(int(row.get('RONLY') or 0) == 1),
            'combo': '' if object_type == 'SEPARATOR' else str(row.get('COMBO') or '').strip(),
            'virtual': None,
            'ordem': int(row.get('ORDEM') or 0),
            'tam': 100 if object_type == 'SEPARATOR' else int(row.get('TAM') or 5),
            'ordem_mobile': int(row.get('ORDEM_MOBILE') or 0),
            'tam_mobile': 40 if object_type == 'SEPARATOR' else int(row.get('TAM_MOBILE') or 5),
            'condicao_visivel': str(row.get('CONDICAO_VISIVEL') or '').strip(),
            'visivel': bool(int(row.get('VISIVEL') or 0) == 1),
            'obrigatorio': False if object_type == 'SEPARATOR' else bool(int(row.get('OBRIGATORIO') or 0) == 1),
            'precisao': None,
            'decimais': 0 if object_type == 'SEPARATOR' else int(row.get('DECIMAIS') or 0),
            'minimo': None if object_type == 'SEPARATOR' or row.get('MINIMO') is None else str(row.get('MINIMO')).strip(),
            'maximo': None if object_type == 'SEPARATOR' or row.get('MAXIMO') is None else str(row.get('MAXIMO')).strip(),
            'is_menu_object': True,
            'ui_only': True,
            'propriedades': props,
        })
