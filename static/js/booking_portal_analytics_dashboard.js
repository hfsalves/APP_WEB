(function () {
  ...
  function diagnostics(items, traffic) { const host = $("#pbDiagnosticsRows"); if (!host) return; host.replaceChildren(); if (!items.length) { host.appendChild(make("p", "pb-card-footnote", ["bots", "suspected", "all"].includes(traffic) ? "Sem tráfego desta qualidade neste período." : "Escolha “Suspeitos”, “Bots” ou “Todos” para consultar o diagnóstico técnico.")); return; } items.slice(0, 20).forEach(item => { const row = make("div", "pb-diagnostic-row"); row.append(make("strong", "", classLabels[item.traffic_class] || item.traffic_class), make("span", "", `${item.reason || "sem razão"} · ${country(item.country)}`), make("span", "", item.requests === null ? `${item.pages} páginas · ${seconds(item.active_seconds)}` : `${value(item.requests)} requests · ${item.pages}`)); host.appendChild(row); }); }
  function activityChart(items) {
    ...
    const max = Math.max(1, ...data.flatMap(item => [Number(item.estimated_visitors) || 0, Number(item.bookings) || 0]));
    ...
    polyline("estimated_visitors", "pb-chart-visitor"); polyline("bookings", "pb-chart-booking");
    ...
    data.forEach((item, index) => { if (!Number(item.estimated_visitors)) return; const dot = document.createElementNS(ns, "circle"); dot.setAttribute("cx", x(index)); dot.setAttribute("cy", y(item.estimated_visitors)); dot.setAttribute("r", 3); dot.setAttribute("class", "pb-chart-dot"); const title = document.createElementNS(ns, "title"); title.textContent = `${item.date}: ${value(item.estimated_visitors)} visitantes estimados; ${value(item.bookings)} reservas concluídas`; dot.appendChild(title); svg.appendChild(dot); });
  ...
  function render(data) {
    ...
    ranked("#pbEstimatedCountryList", data.estimated_countries, item => country(item.key)); ranked("#pbDeviceList", data.devices, item => deviceLabels[item.key] || item.label);
    ranked("#pbBrowserList", data.browsers); ranked("#pbOsList", data.operating_systems); ranked("#pbLanguageList", data.languages);
    ranked("#pbPageList", data.pages); ranked("#pbCheckinMonthList", []); activityChart(data.timeline);
    properties(data.properties); sessionRows(data.recent_sessions); funnel(data.funnel); diagnostics(data.traffic_quality.diagnostics, data.filters.traffic);
    const request = $("#pbTechnicalRequests"); if (request) request.textContent = `${value(data.kpis.requests)} requests técnicos no período. Bots e suspeitos só aparecem no filtro respetivo.`;
    const quality = $("#pbTrafficQuality"); if (quality) quality.textContent = ["bots", "suspected", "all"].includes(data.filters.traffic) ? (data.traffic_quality.request_classes || []).map(x => `${classLabels[x.key] || x.key}: ${value(x.value)}`).join(" · ") || "Sem requests." : "A estimativa principal exclui bots e tráfego suspeito.";
