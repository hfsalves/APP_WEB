...
document.addEventListener('DOMContentLoaded', () => {
  ...
  console.log('IS_LP_ADMIN =', typeof IS_LP_ADMIN !== 'undefined' ? IS_LP_ADMIN : '(undefined)');
  ...
  async function fetchUsersList() {
    // Se não for admin nem LP admin, apenas o próprio
    const isLpAdmin = Number(window.IS_LP_ADMIN || 0) === 1;
    const fallbackFromTasks = () => {
      const set = new Set();
      try {
        if (Array.isArray(window._ALL_TASKS)) {
          window._ALL_TASKS.forEach(t => {
            const u = (t && t.UTILIZADOR ? String(t.UTILIZADOR) : '').trim();
            if (u) set.add(u);
          });
        }
      } catch (_) {}
    ...
    if (!IS_ADMIN && !isLpAdmin) return [CURRENT_USER];
    try {
      let list = [];
      ...
      try {
        if (Array.isArray(window._ALL_TASKS)) {
          window._ALL_TASKS.forEach(t => {
            const u = (t.UTILIZADOR || '').toString().trim();
            if (u) set.add(u);
          });
        }
      } catch (_) {}
      set.add(CURRENT_USER);
      cacheUsers = Array.from(set).filter(Boolean).sort((a,b)=>a.localeCompare(b));
      try { console.log('[monitor] users fetched for filtros:', cacheUsers); } catch(_) {}
      return cacheUsers;
    } catch (err) {
      ...
      try {
        ...
        console.error('[monitor] fetch alternativo users falhou', err2);
      }
      // Fallback final: cache atual ou utilizadores das tarefas
      if (Array.isArray(cacheUsers) && cacheUsers.length) return cacheUsers;
      cacheUsers = fallbackFromTasks();
      return cacheUsers;
    }
  }
