...
each pre-invoice. It never changes the BC -> BL -> PF BI.OBISTAMP chain.
"""
...
def repair_headers_and_lines(
    ...
    *Equipe*. A PF must leave both fields blank. A BL must leave FREF blank
    ...
    return pf_headers, bl_headers, pf_lines
    ...
    with app.app_context():
        with pyodbc.connect(_phc_conn_str(PHC_DB, PHC_SERVER), timeout=60) as connection:
            ...
            if not args.execute:
                ...
                return 0
            headers, bl_headers, lines = repair_headers_and_lines(cursor, documents, bl_stamps)
            ...
            print(
                ...
                f"{lines} linhas corrigidas, {attached} PDFs anexados."
            )
            if attach_missing:
